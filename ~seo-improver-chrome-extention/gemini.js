...
/**
 * Конфигурация API endpoints для разных версий Gemini API.
 */
const API_ENDPOINTS = {
    v1: {
...
/**
 * Формирует URL для запроса к Gemini API в зависимости от версии.
 * @param {string} model Название модели (например, 'gemini-2.5-flash-lite').
 * @param {string} apiKey API ключ для аутентификации.
 * @param {string} apiVersion Версия API ('v1' или 'v2'). По умолчанию 'v1'.
 * @returns {string} Сформированный URL для API запроса.
 */
function _buildApiUrl(model, apiKey, apiVersion = 'v1') {
    const endpoint = API_ENDPOINTS[apiVersion];

    if (!endpoint) {
        logger.warn(`[GEMINI] Неизвестная версия API: "${apiVersion}". Используется v1 по умолчанию.`);
        return _buildApiUrl(model, apiKey, 'v1');
    }

    const encodedModel = encodeURIComponent(model);
    const encodedKey = encodeURIComponent(apiKey);

    return `${endpoint.baseUrl}/${encodedModel}:${endpoint.method}?key=${encodedKey}`;
}

/**
 * Отправляет запрос к Gemini API и возвращает текстовый ответ.
 * @param {string} fullPrompt Полный текст промпта для отправки модели.
 * @param {string} apiKey API ключ для аутентификации.
 * @param {string} model Название модели Gemini.
 * @param {string} apiVersion Версия API ('v1' или 'v2'). По умолчанию 'v1'.
 * @returns {Promise<string>} Текстовый ответ от модели.
 * @throws {Error} При ошибке соединения или получения ответа от API.
 */
async function _sendRequestToGemini(fullPrompt, apiKey, model, apiVersion = 'v1') {
    const url = _buildApiUrl(model, apiKey, apiVersion);

    try {
        logger.info(`[GEMINI] Отправка запроса к API (версия: ${apiVersion})`);
        ...
        // Проверка наличия кандидатов ответа
        if (!data.candidates || data.candidates.length === 0) {
            ...
            error.details = data.promptFeedback;
            throw error;
        }
        ...
        logger.info('[GEMINI] Успешно получен ответ от модели');
        return resultText;
...
/**
 * Получает полное предложение по цене от модели Gemini.
 * @param {string} pageText Текст страницы для анализа.
 * @param {string} apiKey API ключ для аутентификации.
 * @param {string} model Название модели Gemini.
 * @param {string} apiVersion Версия API ('v1' или 'v2'). По умолчанию 'v1'.
 * @returns {Promise<string>} Текстовый ответ от модели с предложением по цене.
 * @throws {Error} При ошибке загрузки инструкций или выполнения запроса.
 */
GeminiAPI.getFullPriceOffer = async (pageText, apiKey, model, apiVersion = 'v1') => {
    ...
    return await _sendRequestToGemini(fullPrompt, apiKey, model, apiVersion);
};

/**
 * Получает ответ модели в формате JSON.
 * @param {string} pageText Текст страницы для анализа.
 * @param {string} apiKey API ключ для аутентификации.
 * @param {string} model Название модели Gemini.
 * @param {string} apiVersion Версия API ('v1' или 'v2'). По умолчанию 'v1'.
 * @returns {Promise<Object>} Распарсенный JSON ответ от модели.
 * @throws {Error} При ошибке парсинга JSON или выполнения запроса.
 */
GeminiAPI.getModelResponseJSON = async (pageText, apiKey, model, apiVersion = 'v1') => {
    const modelResponse = await GeminiAPI.getFullPriceOffer(pageText, apiKey, model, apiVersion);
